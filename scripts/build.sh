...
echo -e "${BLUE}🦉 Recon Build Script${NC}"
echo "================================"
...
uv run pyinstaller recon.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="recon-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/recon.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/recon.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/recon" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/recon" "$RELEASE_DIR/$BINARY_NAME"
